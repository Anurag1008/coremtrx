...
export function SectionHeader({ label, title, subtitle, centered = false }: SectionHeaderProps) {
  return (
    <div className={centered ? "text-center max-w-2xl mx-auto" : ""}>
      ...
      <h2 className="font-[Syne] font-extrabold text-[clamp(2rem,4vw,3rem)] tracking-tight mb-3">{title}</h2>
      {subtitle && (
        <p className="text-[#6b7a99] text-base leading-relaxed font-light max-w-xl">{subtitle}</p>
      )}
    </div>
  );
}
